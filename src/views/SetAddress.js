...
const SetAddress = ({navigation, route}) => {
  ...
  const onCraeteOrder = async () => {
    ...
    if (response.statusCode == 200) navigation.navigate('TabScreens');
  ...
  return (
    <ScrollView>
      <SafeAreaView
        ...
        <Text style={{fontSize: 22, color: "#0EC83B" , fontWeight: 'bold'}}>
        สินค้าที่ต้องชำระ
        ...
        {data.map((item, i) => (
          <View
            ...
            <View>
              <Text style={{padding: 5, fontSize: 18}}>{item.productName}</Text>
